$(document).ready(function () {
    $("#search-rekomendasi").on("click", function () {
        let form = $("#form-rekomendasi").serialize();
        $("#spinner").html(loader)
        $.ajax({
            data: form,
            url: "/electre-normalisasi-user",
            type: "GET",
            dataType: 'json',
            success: function (response) {
                ...
                $.ajax({
                    data: { data: response.hasil, alternatifs: response.alternatifs },
                    url: "/electre-normalisasi-bobot-user",
                    type: "GET",
                    dataType: 'json',
                    success: function (response) {
                        // console.log(response.alternatifs);
                        ...
                        $.ajax({
                            data: { data: response.hasil, alternatifs: response.alternatifs },
                            url: "/concordance-electre-user",
                            type: "GET",
                            dataType: 'json',
                            success: function (response) {
                                let data = response.hasil;
                                let alternatif = response.alternatif;
                                let alternatifs = response.alternatifs;
                                ...
                                $.ajax({
                                    data: { array: collect_dis, alternatifs: alternatifs },
                                    url: "/matrix_dis-user",
                                    type: "GET",
                                    dataType: 'json',
                                    success: function (response) {
                                        ...
                                        let tableRanking = `<div class="row">
                                                                       <div class="col-sm-12">
                                                                           <div class="card">
                                                                               <div class="card-header">
                                                                               <h2>Rekomendasi Tempat Wisata Untuk Anda</h2>
                                                                               </div>
                                                                               <div class="card-body">
                                                                               <table class='table table-bordered table-hover dtr-inline'>
                                                                                <thead>
                                        ...
                                        // /RANKING
                                        for (h = 0; h < sum_array_c.length; h++) {
                                            rankTable.push([`A${h + 1}`, sum_array_c[h], sum_array_d[h], (parseFloat(sum_array_c[h]) - parseFloat(sum_array_d[h])).toFixed(4), final_rank[h], h + 1])
                                        }
                                        let tableBaru = sortedArray(rankTable, 3);
                                        console.log(alternatifs);
                                        let pilihan = [];
                                        for (h = 0; h < tableBaru.length; h++) {
                                            var terpilih = alternatifs.find(function (alternatif) {
                                                return alternatif.alternatif == tableBaru[h][5];
                                            });
                                            pilihan.push(terpilih.keterangan)
                                        }

                                        for (z = 0; z < pilihan.length; z++) {
                                            if (pilihan[z] != 'Pilihan Anda') {
                                                tableRanking += `<tr>`;
                                                if (pilihan[z - 1] == 'Pilihan Anda') {
                                                    tableRanking += `<td class="text-success">${pilihan[z]}&nbsp;(Rekomendasi)</td>`;
                                                    Swal.fire(pilihan[z], "Adalah Rekomendasi Tempat Wisata Anda", "success");
                                                } else {
                                                    tableRanking += `<td>${pilihan[z]}</td>`;
                                                }
                                                tableRanking += `</tr>`;
                                            }
                                            if (pilihan[z] == 'Pilihan Anda' && z + 1 == alternatifs.length) {
                                                Swal.fire("Warning!", "Tidak Ada Rekomendasi", "warning");
                                            }
                                        }
